...
import random
from flask import request, redirect   #ended up not using redirect because it would make a loop and my template would not reach the rendering stage
import serial 
import requests
import json
...
def get_info():         #step to define get route
    ...
    data = json.loads(ser.readline().decode().strip())  #the data variable for template needs to be a json file. read the serial input decode strip and jsonify it. without json loads it becomes strings! bad!
    data["Time"]=timeonly           #this is necessary to make timestamps on the template. in data there is a time placeholder in template. relate it to timeonly variable
    requests.post("http://127.0.0.1:5000/addData", json = data ) #post the json data 
    
    return render_template ("exercise1.html", time = timeonly, time_day= current_time, test=infolist ) #display all the data with the placeholders and the loop. test is where the jsonfiles go
...
def post_info():
    ...
    infolist.append(thing)      #add thing(jsonfiles) into that empty list. I need it for the template
